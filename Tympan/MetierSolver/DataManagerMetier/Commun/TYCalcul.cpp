...
int TYCalcul::fromXML(DOM_Element domElement)
{
    ...
    // CLM-NT33: Compatibilti申 ancien format XML
    ...
    for (i = 0; i < childs.length(); i++)
    {
        ...
        // CLM-NT33: Compatibiliti申 avec ancien format XML
        // Points de contri申le
...
void TYCalcul::addToSelection(TYElement* pElt, bool recursif /*=true*/)
{
    ...
    if (addToSelection(id))
    {
        ...
        else if (pElt->inherits("TYSiteNode"))
        {
            ...
            // Si c'est un site on ajoute pas systi申matiquement tous les enfants
            recursif = false;
        }
...
bool TYCalcul::remToSelection(TYElement* pElt, bool recursif /*=true*/)
{
    ...
    // Si c'est un siteNode, on fait une mise i申 jour de l'altimi申trie
...
void TYCalcul::selectActivePoint(const LPTYSiteNode pSite)
{
    ...
    // On commence par recuperer la liste des machines et des bi申timents
    // Bi申timents
    for (j = 0; j < static_cast<int>(pInfra->getListBatiment().size()); j++)
    {
        // Si ce bi申timent est actif pour le calcul
...
bool TYCalcul::go()
{
    ...
    setIsAcousticModified(true); // Les donni申es acoustiques ont i申ti申 actualisi申es
