...
bool TYMaillage::sumArith(const TYMaillage& maillage, TYMaillage& maillageOut) const //, TYMaillage& maillageOut) const
{
    if (!TYMaillage::checkSimilar(this, &maillage))
    {
        return false;
    }

    maillageOut.deepCopy(this, false);
    for (unsigned int j = 0; j < getPtsCalcul().size(); j++)
    {
        TYSpectre spectre2 = *maillage.getPtsCalcul()[j]->getSpectre();
        TYSpectre Spectre(getPtsCalcul()[j]->getSpectre()->sum(spectre2));
        LPTYPointCalcul pPtCalcul = new TYPointCalcul();
        pPtCalcul->deepCopy(getPtsCalcul()[j], false);
        pPtCalcul->setSpectre(Spectre);
        maillageOut.getPtsCalcul()[j] = pPtCalcul;
    }

    return true;
}

bool TYMaillage::sumEnerg(const TYMaillage& maillage, TYMaillage& maillageOut) const
{
    if (!TYMaillage::checkSimilar(this, &maillage))
    {
        return false;
    ...
    {
        TYSpectre spectre2 = *maillage.getPtsCalcul()[j]->getSpectre();
        TYSpectre Spectre(getPtsCalcul()[j]->getSpectre()->sumdB(spectre2));
        LPTYPointCalcul pPtCalcul = new TYPointCalcul();
        pPtCalcul->deepCopy(getPtsCalcul()[j], false);
        pPtCalcul->setSpectre(Spectre);
        maillageOut.getPtsCalcul()[j] = pPtCalcul;
...
bool TYMaillage::substArith(const TYMaillage& maillage, TYMaillage& maillageOut) const
{
    if (!TYMaillage::checkSimilar(this, &maillage))
    {
        return false;
    ...
    {
        TYSpectre spectre2 = *maillage.getPtsCalcul()[j]->getSpectre();
        TYSpectre Spectre(getPtsCalcul()[j]->getSpectre()->subst(spectre2));
        LPTYPointCalcul pPtCalcul = new TYPointCalcul();
        pPtCalcul->deepCopy(getPtsCalcul()[j], false);
        pPtCalcul->setSpectre(Spectre);
        maillageOut.getPtsCalcul()[j] = pPtCalcul;
...
bool TYMaillage::substEnerg(const TYMaillage& maillage, TYMaillage& maillageOut) const
{
    if (!TYMaillage::checkSimilar(this, &maillage))
    {
        return false;
    ...
    {
        TYSpectre spectre2 = *maillage.getPtsCalcul()[j]->getSpectre();
        TYSpectre Spectre(getPtsCalcul()[j]->getSpectre()->substdB(spectre2));
        LPTYPointCalcul pPtCalcul = new TYPointCalcul();
        pPtCalcul->deepCopy(getPtsCalcul()[j], false);
        pPtCalcul->setSpectre(Spectre);
        maillageOut.getPtsCalcul()[j] = pPtCalcul;
...
bool TYMaillage::multCoef(const double& coeff, TYMaillage& maillageOut) const
{
    maillageOut.deepCopy(this, false);
    for (unsigned int j = 0; j < getPtsCalcul().size(); j++)
    {
        TYSpectre Spectre(getPtsCalcul()[j]->getSpectre()->mult(coeff));
        LPTYPointCalcul pPtCalcul = new TYPointCalcul();
        pPtCalcul->deepCopy(getPtsCalcul()[j], false);
        pPtCalcul->setSpectre(Spectre);
        maillageOut.getPtsCalcul()[j] = pPtCalcul;
