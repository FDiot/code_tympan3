        ...
        for (i = 0; i < _pInfrastructure->getListBatiment().size(); i++)
        {
            ...
            if (pBatiment && pBatiment->isInCurrentCalcul())
            {
                ...
                for (j = 0; j < pBatiment->getTabAcousticVol().size(); j++)
                {
                    ...
                    if (pEtage)
                    {
                        //Ri申cupi申ration des faces des murs
                        for (unsigned int k = 0; k < pEtage->getTabMur().size(); k++)
                        {
                            ...
                            if (mur)
                            {
                                file << "Ri申cupi申ration d'un mur rectangulaire." << endl;
                                ...
                                if (pRect)
                                {
                                    ...
                                    file << "Ri申cupi申ration d'un rectangle." << endl;
...
bool TYSiteNode::update(TYElement* pElem)
{
    ...
    {
        return true; // Pas de mise i申 jour ni申cessaire
    }
    else if (pElem->inherits("TYAcousticLine")) // Cas 1 : un objet de type source lini申ique
    {
        TYAcousticLine::safeDownCast(pElem)->updateAcoustic(true);
    }
    else // Autres cas, recherche de parent et traitement approprii申
    {
        ...
        do
        {
            if (pParent->inherits("TYAcousticVolumeNode"))
            {
                ...
                break; // On a trouvi申, on peut sortir
            }
...
void TYSiteNode::update(const bool& force) // Force = false
{
    ...
    // Si le site est dans un projet, on altimi申trise les points de controle
    if (_pProjet)
    {
        TYCalcul* pCalcul = _pProjet->getCurrentCalcul()._pObj;
        assert(pCalcul);
...
void TYSiteNode::appendSite(LPTYSiteNode pSiteFrom, const OMatrix& matrix, LPTYSiteNode pSiteTo)
{
    ...
    // BUG #0008309 : Courbe de niveau pas prise en compte dans l'altimi申trie
