        ...
        for (k = 0; k < ray->events.size(); k++) // boucle sur les evenements du rayon
        {
            ...
            if (ev.type == SPECULARREFLEXION) // cas d'une reflexion
            {
                ...
                if (zoneFresnel)
                {
                    ...
                    for (l = 0; l < nbFacesFresnel; l++) // boucle sur les faces = intersection plan de l'objet intersecti�� / ellipsoide de Fresnel
                    {
                        sum = sum + coefRefl * tabPondFresnel[l];    // calcul du coeff de reflexion moy en ponderant avec les mati��riaux
                    }
