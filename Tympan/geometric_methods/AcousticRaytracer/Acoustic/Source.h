...
public:
#ifdef _ALLOW_TARGETING_
    Source(std::string _name = "unknown source") :  sampler(NULL), Base(), targetManager(NULL) { name = _name;}
...
#else
    /// Base constructor
    Source(std::string _name = "unknown source") :  sampler(NULL), Base() { name = _name;}
    /// Copy constructor
    Source(const Source& other) : Base(other)
    {
        name = std::string(other.name);
