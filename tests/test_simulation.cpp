...
void setup_random(Simulation* simu,
	...
	while(recepteurs.size()<nbRecepteurs){
		...
		vec3 pos=vec3(rand() % 1000 + 1,rand() % 1000 + 1,rand() % 1000 + 1);
...
TEST(test_simulation_1source_1recepteur, test_source)
{
	...
	// Test name, id, pos, initial ray count and number of rays left
	EXPECT_EQ("src1",source.getName());
	EXPECT_EQ(0,source.getId());
	EXPECT_TRUE(vec3(5,0,0)==source.getPosition());
	EXPECT_EQ(10,source.getInitialRayCount());
	EXPECT_EQ(0,source.getNbRayLeft());
...
TEST(test_simulation_1source_1recepteur, test_recepteur)
{
	...
	EXPECT_EQ("rcpt1",recepteur.getName());
	EXPECT_EQ(recepteur.getId(),0);
	EXPECT_TRUE(vec3(15,0,0)==recepteur.getPosition());
...
TEST(test_simulation_1source_1recepteur, test_valid_ray)
{
	...
	EXPECT_EQ(source.getName(),ray_source->getName());
	EXPECT_TRUE(source.getPosition()==ray_source->getPosition());
	EXPECT_EQ(source.getId(),ray_source->getId());

	// Test original receptor versus valid_ray receptor
	EXPECT_EQ(recepteur.getName(),ray_recepteur->getName());
	EXPECT_TRUE(recepteur.getPosition()==ray_recepteur->getPosition());
	//EXPECT_EQ(recepteur.getId(),ray_recepteur->getId()); //l'id de ray_recepteur est invalid

	// Test ray 
	EXPECT_EQ(0,valid_ray->getNbEvents());
	EXPECT_EQ(0,valid_ray->getDiff());
	EXPECT_EQ(0,valid_ray->getReflex());
	EXPECT_EQ(10,valid_ray->getLongueur());
...
TEST(test_simulation_1source_1recepteur, test_diffraction)
{
	...
	// Setup
	setup_1source_1recepteur(&simu,100);
	...
	std::deque<Ray*>* diff_rays=new std::deque<Ray*>();

	for(int i=0;i<debug_rays->size();i++){
		Ray* ray=debug_rays->at(i);
		std::vector<QSharedPointer<Event> >* events=ray->getEvents();
		for(int j=0;j<events->size();j++){
			QSharedPointer<Event> e=events->at(j);
			if(e.value->getType()==DIFFRACTION){
				diff_rays->push_back(ray);
				break;
			}
		}
	}
...
TEST(test_simulation_random_init, test_sources)
{
	...
	EXPECT_EQ(sources.size(),sources_after_simulation.size());
	...
	for(unsigned int i=0;i<sources.size();i++){
		...
		//  Test source
		EXPECT_EQ(src_original.getName(),src_after_simulation.getName());
		EXPECT_EQ(src_original.getId(),src_after_simulation.getId());
		EXPECT_TRUE(src_original.getPosition()==src_after_simulation.getPosition());
		EXPECT_EQ(nbRaysPerSource,src_after_simulation.getInitialRayCount());
		EXPECT_EQ(0,src_after_simulation.getNbRayLeft());
...
TEST(test_simulation_random_init, test_recepteurs)
{
	...
	EXPECT_EQ(recepteurs.size(),recepteurs_after_simulation.size());
	...
	for(unsigned int i=0;i<recepteurs.size();i++){
		...
		//  Test recepteur
		EXPECT_EQ(rcpt_original.getName(),rcpt_after_simulation.getName());
		EXPECT_EQ(rcpt_original.getId(),rcpt_after_simulation.getId());
		EXPECT_TRUE(rcpt_original.getPosition()==rcpt_after_simulation.getPosition());
...
TEST(test_simulation_random_init, test_valid_rays_nbRaysPerSource_equals_0)
{
	...
	for(unsigned int i =0;i<valid_rays->size();i++){
		...
		// Test original source versus valid_ray source 
		EXPECT_EQ(sources.at(ray_source->getId()).getName(),ray_source->getName());
		EXPECT_TRUE(src_pos==ray_source->getPosition());

		// Test original receptor versus valid_ray receptor
		EXPECT_EQ(recepteurs.at(ray_recepteur->getId()).getName(),ray_recepteur->getName());
		EXPECT_TRUE(rcpt_pos==ray_recepteur->getPosition());

		// Test ray 
		EXPECT_EQ(0,valid_ray->getNbEvents());
		EXPECT_EQ(0,valid_ray->getDiff());
		EXPECT_EQ(0,valid_ray->getReflex());
		EXPECT_EQ(src_pos.distance(rcpt_pos),valid_ray->getLongueur());
	}
